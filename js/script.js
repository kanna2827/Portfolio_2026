...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const lightingCards = document.querySelectorAll(
        '.interactive-lighting-card, .hero-glass-card, .glass-card, .bento-card, .counter-card, .timeline-content, .project-card, .intern-card, .training-card, .cert-card, .contact-card, .about-highlight-card'
    );
    const activeCards = new Set();
    let isLoopRunning = false;
    const ease = 0.15; // Linear interpolation factor (speed of flashlight glide)
    ...
    lightingCards.forEach(card => {
        ...
        card.addEventListener('mouseenter', (e) => {
            ...
            // Instantly position light at entry point
